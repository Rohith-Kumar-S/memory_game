...
def reduce_to_relative_asset_path(card_path):
    game_directory = os.path.dirname((os.path.dirname(os.path.dirname(card_path))))
...
def pick_one_cards_from_asset_list(assets):
    menu_cards_list_dict = {}
    key = 0
    for asset in assets:
        file = os.listdir(asset).pop()
